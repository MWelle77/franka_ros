 ...
 */
struct CustomFrankaDataContainerKthJointAcc {
...
class KthJointAccelerationEffortInterfaceController: public controller_interface::MultiInterfaceController<
 ...
 private:
  std::map<std::string, CustomFrankaDataContainerKthJointAcc>  arms_data_;  ///< Holds all relevant data for both arms.
   ...
   */
  Eigen::Matrix<double, 7, 1> saturateTorqueRate( const CustomFrankaDataContainerKthJointAcc& arm_data, const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
   ...
   */
  void updateArm(CustomFrankaDataContainerKthJointAcc& arm_data, const string& arm_id, const ros::Duration& period);
   ...
   */
  void startingArm(CustomFrankaDataContainerKthJointAcc& arm_data);
