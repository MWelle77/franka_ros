 ...
 */
struct CustomFrankaDataContainerKthJointVel {
...
class KthJointVelocityEffortInterfaceController: public controller_interface::MultiInterfaceController<
 ...
 private:
  std::map<std::string, CustomFrankaDataContainerKthJointVel>  arms_data_;  ///< Holds all relevant data for both arms.
   ...
   */
  Eigen::Matrix<double, 7, 1> saturateTorqueRate( const CustomFrankaDataContainerKthJointVel& arm_data, const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
   ...
   */
  void updateArm(CustomFrankaDataContainerKthJointVel& arm_data, const string& arm_id, const ros::Duration& period);
   ...
   */
  void startingArm(CustomFrankaDataContainerKthJointVel& arm_data);
  ...
  void targetJointVelCallback(const geometry_msgs::PoseStamped::ConstPtr& msg, const string& robot_id); 
