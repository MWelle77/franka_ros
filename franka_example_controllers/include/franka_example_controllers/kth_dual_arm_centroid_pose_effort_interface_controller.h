 ...
 */
struct CustomFrankaDataContainerDualArmPose {
 ...
 */
class KthDualArmCentroidPoseEffortInterfaceController: public controller_interface::MultiInterfaceController<
 ...
 private:
  std::map<std::string, CustomFrankaDataContainerDualArmPose>  arms_data_;  ///< Holds all relevant data for both arms.
...
//  ros::Subscriber sub_target_formation_displ_; 
   ...
   */
  Eigen::Matrix<double, 7, 1> saturateTorqueRate( const CustomFrankaDataContainerDualArmPose& arm_data, const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
   ...
   */
  void updateArm(CustomFrankaDataContainerDualArmPose& arm_data, const string& arm_id, const ros::Duration& period);
   ...
   */
  void startingArm(CustomFrankaDataContainerDualArmPose& arm_data);
